...
**2.5.1 Bulk rolls and partial-roll ordering (James, 2026-09-23).** A bulk roll is a strip master of 50m or more (`is_bulk_master`). It is detected by (1) the strip-family parser, (2) a `-NNNM` SKU suffix, or (3) a plain `-NNN` length suffix that the product name confirms in metres (e.g. `LEDRGBFLEX-120-100` "(100m (328ft))"). Pass 3 is needed because the per-foot cut is often on a BOM, so the family parser skips it. **Partial rolls apply ONLY to supplier Neonica Polska Sp. z o.o.**: the reorder quantity is a decimal of the roll (40m needed → 0.40 of a 100m roll), rounded to the nearest 10m. Under 5m needed = no order; 5–15m rounds up to 10m. Every other supplier orders whole rolls, and supplier config cannot override this: once 5m or more is needed, order the next whole roll up (0.4 needed → 1). For every bulk roll (any supplier), stock under 5m is treated as zero, dormancy is measured in metres, and the one-unit range floor does not apply.
...
- **📈 Trend** — momentum >1.5 plus broad customer spread, or sustained monthly lift in the existing 12-month sparkline. The 45d version is either **customers_45d ≥ 10 with top_cust_pct < 50% (and top-2 < 70%)** OR **customers_45d ≥ 3**, **top_cust_pct < 40%**, and **non_top_avg_units ≥ 2**. The monthly version upgrades Stable to Trend when the most recent 3 calendar buckets materially exceed the previous 3 buckets with at least 3 recent customers (10+ buyers also need top_cust_pct < 50%). Real broad-based acceleration. Engine planning rate = last-45d rate, except: if one buyer took ≥40% of the 45d units, their spike is replaced by their own 12mo run rate (others' 45d rate + top buyer's 12mo rate); and the rate is capped at 3× the 12mo rate. Head-count alone is not spread: 2026-09-18 LED-C8020021-2 had 10 buyers but one took 85% of 212 units and the engine suggested 193 on a 1/day product — that shape is now 🔀 Mixed.
