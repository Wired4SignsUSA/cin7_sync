"""Outsourced-order documents (James, 2026-09-08; finishing flow 2026-09-10).
...
will consume:
  ...
  labels     — 2.25 x 1.25 in, one label per finished unit (SKU, name,
               ...
               (corner flow only)
  vendor sheet — All Star finishing instructions: process + colour groups,
               end product, qty, raw profile sent, total feet / end caps,
               PO number. (finishing flow only)
...
def collect(draft_id: int, flow=None) -> dict:
    """{order, flow, po_number,
        assemblies:[{number, sku, name, qty, comps:[(sku,name,total)],
                     services:[(sku,name,total)]}],
        totals:{sku:{name,total,loc,on_hand}}}"""
    import db
    import fablab_assemblies as fa
    from outsource_flows import flow_for_draft
    d = db.get_po_draft(draft_id)
    if not d:
        raise SystemExit(f"order #{draft_id} not found")
    flow = flow or flow_for_draft(d)
    ...
    for a in db.list_fablab_assemblies(draft_id):
        ...
        for ln in task.get("OrderLines", []) or []:
            ...
            if flow.is_service(csku):
                services.append((csku, cname, tot))
                continue
            if fa._is_labor(csku):
                continue
        ...
        rows.append({"number": a["assembly_number"], "sku": a["sku"],
                     ...
                     "qty": qty, "comps": comps, "services": services,
    ...
    return {"order": d, "flow": flow, "po_number": po_number,
            "assemblies": rows, "totals": totals}
...
def write_pick_list(data: dict, path: Path) -> Path:
    ...
    vendor = data["flow"].short
    story = [Paragraph(f"{vendor} pick list — {data['po_number'] or 'PO pending'}", h),
             ...
             Paragraph("Pick the <b>whole lengths</b> below from the locator shown and hand "
                       f"over to {vendor}. Tick each line as picked. Shortages: note in the "
    ...
    story += [tbl, Spacer(1, 6),
              ...
              Spacer(1, 14),
              Paragraph(f"Breakdown by assembly (what {vendor} {data['flow'].what_vendor_does})",
                        styles["Heading2"])]
    ...
    story += [tbl, Spacer(1, 18),
              Paragraph("Picked by: ______________________   Date: ____________   "
                        f"Handed to {vendor} (sign): ______________________", styles["Normal"])]
    doc.build(story)
    return path


# ---------------------------------------------------------------------------
# vendor instruction sheet (finishing flow)
# ---------------------------------------------------------------------------

def write_vendor_sheet(data: dict, path: Path) -> Path:
    """One page for All Star: what to coat/anodize, which colour, how
    many pieces, total feet / end caps, PO number. Grouped by process +
    colour so each batch can be racked together. Built from the service
    lines CIN7 put on each assembly (James, 2026-09-10)."""
    from outsource_flows import describe_finishing_service, parse_finishing_service
    styles = getSampleStyleSheet()
    small = ParagraphStyle("small", parent=styles["Normal"], fontSize=8, leading=10)
    flow = data["flow"]
    d = data["order"]
    po = data["po_number"] or "PO pending"
    doc = SimpleDocTemplate(str(path), pagesize=letter, leftMargin=0.5 * inch,
                            rightMargin=0.5 * inch, topMargin=0.5 * inch,
                            bottomMargin=0.5 * inch,
                            title=f"{flow.supplier} instructions {po}")
    pieces = sum(r["qty"] for r in data["assemblies"])
    story = [Paragraph(f"{flow.supplier} — finishing instructions — {po}", styles["Heading1"]),
             Paragraph(f"From Wired4Signs USA · order #{d['id']} · {d.get('name') or ''} · "
                       f"{pieces:g} pieces · {date.today():%Y-%m-%d}", styles["Normal"]),
             Paragraph("Please quote the PO number on the invoice and packing slip. "
                       "Each group below is one colour / process — keep groups together. "
                       "Qty = number of pieces; Total = feet of profile or number of "
                       "end caps as noted.", small),
             Spacer(1, 8)]

    groups: dict = {}
    for r in data["assemblies"]:
        raw = ", ".join(f"{c[0]} × {_ceil(c[2])}" for c in r["comps"]) or "—"
        if not r["services"]:
            groups.setdefault(("Finish per BOM", ""), []).append(
                (r["sku"], r["name"], r["qty"], raw, "", ""))
        for ssku, sname, tot in r["services"]:
            p = parse_finishing_service(ssku) or {}
            key = (p.get("process") or sname or ssku, p.get("colour") or "")
            unit = p.get("unit") or ""
            groups.setdefault(key, []).append(
                (r["sku"], r["name"], r["qty"], raw,
                 f"{tot:g}" + (f" {unit}" if unit else ""),
                 describe_finishing_service(ssku, sname)))

    for (process, colour), rows in groups.items():
        title = process + (f" — {colour}" if colour else "")
        story.append(Paragraph(title, styles["Heading2"]))
        body = [["", "Finished SKU (what it becomes)", "Description", "Qty",
                 "Raw profile supplied", "Total", "Service"]]
        for sku, name, qty, raw, tot, svc in rows:
            body.append(["[  ]", sku, Paragraph(name[:95], small), f"{qty:g}",
                         Paragraph(raw, small), tot, Paragraph(svc, small)])
        tbl = Table(body, colWidths=[0.3 * inch, 1.7 * inch, 1.9 * inch, 0.45 * inch,
                                     1.4 * inch, 0.7 * inch, 1.05 * inch], repeatRows=1)
        tbl.setStyle(TableStyle([
            ("FONT", (0, 0), (-1, 0), "Helvetica-Bold", 8),
            ("FONT", (0, 1), (-1, -1), "Helvetica", 8),
    ...
    story += [Spacer(1, 8),
              Paragraph("Special instructions / masking: ______________________________"
                        "______________________________", styles["Normal"]),
              Spacer(1, 14),
              Paragraph("Sent by (W4S): ______________________   Date: ____________   "
                        f"Received by ({flow.short}): ______________________", styles["Normal"])]
    doc.build(story)
    return path
...
def build_docs(draft_id: int, out_dir: Optional[Path] = None, flow=None) -> dict:
    """Pick list always; pack labels (corners) / vendor sheet (finishing)
    per the flow profile. Keys absent when a doc does not apply."""
    from data_paths import OUTPUT_DIR
    out_dir = Path(out_dir or (OUTPUT_DIR / "fablab_docs"))
    out_dir.mkdir(parents=True, exist_ok=True)
    data = collect(draft_id, flow=flow)
    flow = data["flow"]
    tag = data["po_number"] or f"order{draft_id}"
    res = {"pick_list": write_pick_list(data, out_dir / f"{tag}_pick_list.pdf"),
           "po_number": data["po_number"], "order": data["order"], "flow": flow,
           "components": len(data["totals"]), "assemblies": len(data["assemblies"])}
    if flow.pack_labels:
        labels, n = write_labels(data, out_dir / f"{tag}_labels_2.25x1.25.pdf")
        res.update(labels=labels, label_count=n)
    if flow.vendor_sheet:
        res["vendor_sheet"] = write_vendor_sheet(
            data, out_dir / f"{tag}_{flow.short.replace(' ', '_')}_instructions.pdf")
    return res


def post_docs(draft_id: int, channel_id: Optional[str] = None,
              thread_ts: Optional[str] = None, flow=None) -> dict:
    """Build + upload the flow's PDFs to Slack. Returns build summary + errors."""
    import fablab_slack
    res = build_docs(draft_id, flow=flow)
    flow = res["flow"]
    channel_id = channel_id or flow.channel_id
    po = res["po_number"]
    errs = []
    _fid, err = fablab_slack.upload_file(
        ...
        initial_comment=(f":clipboard: *{po} — pick list for W4S warehouse* "
                         ...
                         f"whole lengths + stock locators). Pick, tick, hand over to {flow.short}."))
    if err:
    ...
    if res.get("vendor_sheet"):
        _fid, err = fablab_slack.upload_file(
            res["vendor_sheet"], channel_id, thread_ts=thread_ts,
            title=f"{po} {flow.supplier} instructions",
            initial_comment=(f":page_facing_up: *{po} — instruction sheet for {flow.supplier}* "
                             "(colour / process per end product, qty, feet or end caps). "
                             "Print and send with the raw profiles and the PO."))
        if err:
            errs.append(f"vendor sheet upload: {err}")
    res["errors"] = errs
    return res
